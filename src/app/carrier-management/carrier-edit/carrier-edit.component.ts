...
export class CarrierEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.activatedRoute.params.subscribe((params) => {
      ...
      this.certiOfInExDate = this.datePipe.transform(
        new Date(this.certiOfInExDate),
        'yyyy/MM/dd'
      );
  ...
  getIndustries(searchStr: any) {
    ...
    this.httpService.get(APIparams).subscribe((resp) => {
      if (
        resp['response'] &&
        resp['response'].allSicCode &&
        resp['response'].allSicCode.length > 0
      ) {
  ...
  getCarrierDetails() {
    const url =
      APIURL.envConfig.CARRIERENDPOINTS.getCarrierProfile +
      '?id=' +
      this.carrierId;
    this.httpService.get(url).subscribe(
      (resp) => {
        if (resp['response']) {
          ...
          this.carrierProfile.registrationDate = resp['response']
            .registrationDate
            ? new Date(resp['response'].registrationDate + ' ' + 'UTC')
            : null;
          this.carrierProfile.firstLogin = resp['response'].firstLogin
            ? new Date(resp['response'].firstLogin + ' ' + 'UTC')
            : null;
          this.carrierProfile.lastLogin = resp['response'].lastLogin
            ? new Date(resp['response'].lastLogin + ' ' + 'UTC')
            : null;
          this.carrierProfile.createdAt = resp['response'].createdAt
            ? new Date(resp['response'].createdAt + ' ' + 'UTC')
            : null;
          this.carrierProfile.updatedAt = resp['response'].updatedAt
            ? new Date(resp['response'].updatedAt + ' ' + 'UTC')
            : null;
          localStorage.setItem(
            'carrierName',
            this.carrierProfile.legalName ? this.carrierProfile.legalName : ''
          );
          localStorage.setItem(
            'userId',
            this.carrierProfile.userId ? this.carrierProfile.userId : ''
          );
          localStorage.setItem(
            'carrierId',
            this.carrierProfile.id ? this.carrierProfile.id : ''
          );
          ...
          this.searchCtrl = this.carrierProfile.usSic1987Description
            ? this.carrierProfile.usSic1987Description
            : '';

          if (this.carrierProfile && this.carrierProfile.lane == 0) {
          ...
          if (
            this.carrierProfile.certificateOfInsurance &&
            this.datePipe.transform(
              new Date(this.carrierProfile.insuranceCertificateExpireDate),
              'yyyy/MM/dd'
            ) > this.currentDate
          ) {
            localStorage.setItem(
        ...
        if (
          this.carrierProfile &&
          this.carrierProfile.insuranceCertificateExpireDate != null
        ) {
          if (
            this.datePipe.transform(
              new Date(this.carrierProfile.insuranceCertificateExpireDate),
              'yyyy/MM/dd'
            ) < this.certiOfInExDate
          ) {
        ...
        if (resp['response'].lane == '1') {
          console.log('come in lanes');
          showLane = '2';
          laneSelect = '1';
          this.lanLocation = 2;
          this.checkValueLane = false;
        } else {
          console.log('come else lanes');
          showLane = '1';
          laneSelect = '0';
          this.lanLocation = 1;
  ...
  checkSelectedLanes() {
    if (
      this.carrierProfile &&
      this.carrierProfile.laneId &&
      this.carrierProfile.laneId.length > 0
    ) {
      let newLo: any = {};
      this.checkValueLane = false;
      if (
        this.carrierProfile.laneId.pickUp != '' &&
        this.carrierProfile.laneId.dropOff != ''
      ) {
        this.addOption = true;
        this.locations = [];
  ...
  openEditForm() {
    this.editCompanyInfo = this.fb.group({
      ...
      dotNumber: [
        this.carrierProfile.dotNumber ? this.carrierProfile.dotNumber : '',
        Validators.compose([
          Validators.required,
      ...
      telephone: [
        this.carrierProfile.telephone ? this.carrierProfile.telephone : '',
        Validators.compose([
          Validators.required,
          Validators.maxLength(10),
      ...
      emailAddress: [
        this.carrierProfile.emailAddress
          ? this.carrierProfile.emailAddress
          : '',
        Validators.compose([
          Validators.required,
      ...
      dbaName: [
        this.carrierProfile.dbaName ? this.carrierProfile.dbaName : '',
        Validators.compose([Validators.maxLength(64)]),
      ],
      fax: [
        this.carrierProfile.fax ? this.carrierProfile.fax : '',
        Validators.pattern(/^([0-9()-]+\s?)*$/),
      ],
      einTaxId: [this.carrierProfile.einId ? this.carrierProfile.einId : ''],
      numberOfTrucks: [
        this.carrierProfile.truckTotal ? this.carrierProfile.truckTotal : '',
      ],
      numberOfDrivers: [
        this.carrierProfile.driverTotal == 'Owner Operator'
          ? 1
          : this.carrierProfile.driverTotal
          ? this.carrierProfile.driverTotal
          : '',
      ],
    ...
    this.editPersonalInfo = this.fb.group({
      firstName: [
        this.carrierProfile.firstName ? this.carrierProfile.firstName : '',
        Validators.compose([
          Validators.required,
      ...
      lastName: [
        this.carrierProfile.lastName ? this.carrierProfile.lastName : '',
        Validators.compose([
          Validators.required,
          Validators.maxLength(64),
          Validators.pattern(/^([a-zA-Z0-9]+\s?)*$/),
    ...
    if (
      this.carrierProfile &&
      this.carrierProfile.phyState &&
      this.carrierProfile.phyState.length &&
      this.carrierProfile.phyState.length > 2
    ) {
      let i;
    ...
    if (
      this.carrierProfile &&
      this.carrierProfile.mailingState &&
      this.carrierProfile.mailingState.length > 2
    ) {
      let i;
      for (i = 0; i < this.statesList.length; i++) {
    ...
    this.editAddress = this.fb.group({
      ...
      addressLine1: [
        this.carrierProfile.phyStreet ? this.carrierProfile.phyStreet : '',
        Validators.compose([Validators.required]),
      ],
      addressLine2: [
        this.carrierProfile.phyAddressLine2
          ? this.carrierProfile.phyAddressLine2
          : '',
      ],
      zipShort: [
        this.carrierProfile.phyZipShort ? this.carrierProfile.phyZipShort : '',
        Validators.compose([
          Validators.required,
          Validators.maxLength(10),
          Validators.pattern(/^([0-9]+\s?)*$/),
      ...
      city: [
        this.carrierProfile.phyCity ? this.carrierProfile.phyCity : '',
        Validators.compose([Validators.required]),
      ],
      state: [
      ...
      mailAddLine1: [
        this.carrierProfile.mailingStreet
          ? this.carrierProfile.mailingStreet
          : '',
        Validators.compose([Validators.required]),
      ],
      mailAddLine2: [
        this.carrierProfile.mailingAddressLine2
          ? this.carrierProfile.mailingAddressLine2
          : '',
      ],
      mailZipshort: [
        this.carrierProfile.mailingZipShort
          ? this.carrierProfile.mailingZipShort
          : '',
        Validators.compose([
          Validators.required,
      ...
      mailCity: [
        this.carrierProfile.mailingCity ? this.carrierProfile.mailingCity : '',
        Validators.compose([Validators.required]),
      ],
      mailState: [
        mailStateShort ? mailStateShort : '',
        Validators.compose([Validators.required]),
      ],
    ...
    if (
      this.carrierProfile.phyAddressLine2 ==
        this.carrierProfile.mailingAddressLine2 &&
      this.carrierProfile.mailingStreet == this.carrierProfile.phyStreet &&
      this.carrierProfile.mailingZip == this.carrierProfile.phyZip
    ) {
      this.sameAsPhysicalAdd = true;
    }
    this.editMCS = this.fb.group({
      mcsDate: [
        this.carrierProfile.mcs150Date ? this.carrierProfile.mcs150Date : '',
      ],
      mcsMileage: [
        this.carrierProfile.mcs150Mileage
          ? this.carrierProfile.mcs150Mileage
          : '',
        Validators.compose([
          Validators.pattern(/^([0-9-]+\s?)*$/),
      ...
      mcsMileageYear: [
        this.carrierProfile.mcs150MileageYear
          ? this.carrierProfile.mcs150MileageYear
          : '',
        Validators.compose([
    ...
    this.operations = (
      this.carrierProfile.operationId ? this.carrierProfile.operationId : ''
    ).toString();
    this.equipmentLanesEdit = this.fb.group({
      operations: [
        (this.carrierProfile.operationId
          ? this.carrierProfile.operationId
          : ''
        ).toString(),
        Validators.compose([Validators.required]),
      ...
      date: [
        this.carrierProfile.addDate ? this.carrierProfile.addDate : '',
        Validators.compose([Validators.required]),
      ],
      oicState: [
        this.carrierProfile.oicState ? this.carrierProfile.oicState : '',
        Validators.compose([Validators.required]),
      ],
      equipment: [
        this.carrierProfile.equipmentType
          ? this.carrierProfile.equipmentType
          : '',
        Validators.compose([Validators.required]),
      ],
  ...
  onFileSelected(event) {
    ...
    if (event.target.files.length > 0) {
      ...
      reader.onload = function (e) {
        $('#imgFilter').attr('src', e.target['result']);
      };
  ...
  setSameMailAddress(event) {
    ...
    if (event.checked) {
      this.editAddress
        .get('mailAddLine1')
        .setValue(this.editAddress.get('addressLine1').value);
      this.editAddress
        .get('mailCity')
        .setValue(this.editAddress.get('city').value);
      // this.editAddress.get("mailPostalCode").setValue(this.editAddress.get("postalCode").value);
      this.editAddress
        .get('mailState')
        .setValue(this.editAddress.get('state').value);
  ...
  onSubmit(form1, formName) {
    ...
    this.valid2 =
      (lanedatalength.length && this.preferredLanes == 'preferred') ||
      this.preferredLanes == 'all';
    ...
    if (this.sameAsPhysicalAdd) {
      ...
      this.editAddress
        .get('mailZipshort')
        .setValue(this.editAddress.get('zipShort').value);
      this.editAddress
        .get('mailState')
        .setValue(this.editAddress.get('state').value);
      this.mailingLatitude = this.phyLatitude;
    ...
    if (
      ...
      if (formName == 'companyInfo') {
        ...
        formVal.mcsDate = this.datePipe.transform(
          formVal.mcsDate,
          'MM/dd/yyyy'
        );
      ...
      if (formName == 'editAddress') {
        ...
        if (
          this.carrierProfile.phyLatitude != this.phyLatitude &&
          this.carrierProfile.phyStreet != formVal.addressLine1
        ) {
          formData.append('phyLatitude', this.phyLatitude);
        }
        if (
          this.carrierProfile.phyLongitude != this.phyLongitude &&
          this.carrierProfile.phyStreet != formVal.addressLine1
        ) {
        ...
        if (
          this.carrierProfile.mailingLatitude != this.mailingLatitude &&
          this.carrierProfile.mailingStreet != formVal.mailAddLine1
        ) {
          formData.append('mailingLatitude', this.mailingLatitude);
        }
        if (
          this.carrierProfile.mailingLongitude != this.mailingLongitude &&
          this.carrierProfile.mailingStreet != formVal.mailAddLine1
        ) {
      ...
      if (formName == 'editMCS') {
        ...
        let upcomingDate = this.datePipe.transform(
          this.carrierProfile.mcs150Date,
          'MM/dd/yyyy'
        );
      ...
      if (formName == 'equipmentLanesEdit') {
        ...
        if (
          this.searchCtrlValue &&
          this.carrierProfile.industries != this.searchCtrlValue.id &&
          this.carrierProfile.usSic1987Description != this.searchCtrlValue.value
        ) {
        ...
        if (this.carrierProfile.carrierOperation == 'Interstate') {
          ...
          this.carrierProfile.carrierOperation == 'Intrastate Hazmat'
        ) {
          carrierOperationTitle = '2';
        } else if (
          this.carrierProfile.carrierOperation == 'Intrastate Non-Hazmat'
        ) {
        ...
        if (this.carrierProfile.addDate) {
          compareDate = this.datePipe.transform(
            this.carrierProfile.addDate,
            'MM/dd/yyyy'
          );
        }
  ...
  lanesSelection(event) {
    ...
    if (this.preferredLanes == 'all') this.lanLocation = 2;
    else this.lanLocation = 1;
  }
  ...
  checkOption(event, name, id) {
    ...
    for (let i = 0; i < this.totalLanes; i++) {
      //Same lane issue
      if (
        this.locations[this.totalLanes].pickUp == this.locations[i].pickUp &&
        this.locations[this.totalLanes].dropOff == this.locations[i].dropOff
      ) {
  ...
  getAPIParam(searchStr, type) {
    ...
    if (searchStr) {
      if (type == 'lanes') {
        ...
        APIparams =
          APIURL.envConfig.CARRIERENDPOINTS.lanes +
          '&page=' +
          params.page +
          '&search=' +
          params.search;
      ...
      if (type == 'industries') {
        ...
        APIparams =
          APIURL.envConfig.CARRIERENDPOINTS.industries +
          '?search=' +
          params.search;
      }
    } else {
      if (type == 'lanes') {
        ...
        APIparams =
          APIURL.envConfig.CARRIERENDPOINTS.lanes + '&page=' + params.page;
      }
      if (type == 'industries') {
        params = { limit: 10 };
    ...
    for (i = 0; i < this.equipmentType.length; i++) {
      for (j = 0; j < this.selectedEquipmentType.length; j++) {
        if (this.selectedEquipmentType[j] == this.equipmentType[i]['label']) {
          ...
          this.uncheckedEquipment = this.uncheckedEquipment.filter(
            (e) => e !== this.selectedEquipmentType[j]
          );
          break;
        }
      }
  ...
  removeDocument(documentType: any, id: any) {
    ...
    this.httpService.put(url, APIparams).subscribe(
      (resp) => {
        if (resp['success']) {
